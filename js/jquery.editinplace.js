...
jQuery.fn.editInPlace = function(options) {
	...
	var settings = {
		...
		save_button:		'<button class="inplace_save">Add phrase</button>',
	...
	return this.each(function(){
		...
		.mouseover(function(){
			jQuery(this).css("color", settings.txt_over);
		})
		...
		.mouseout(function(){
			jQuery(this).css("color", settings.txt_out);
		})
		...
		.click(function(){
			...
			if(!editing)
			{
				...
				{
					var use_field_type = '<input style="font-size:70%; width: 80%" type="text" name="inplace_value" class="inplace_field" value="' +
					...
					if(settings.callback) {
						...
						jQuery.ajax({
							...
							success: function(html){
								...
								/* put the old HTML back						*/
								original_element.html(original_html);							
								
								//html = settings.new_html;
								
								if (settings.success) settings.success(new_text);
							},
							error: function(request) {
								original_element.html(original_html);
